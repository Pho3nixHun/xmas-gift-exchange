...
import { Heart, Gift, Plus, Sparkles, ShoppingCart, Check, X } from 'lucide-react';
...
const WishesScreen: React.FC<WishesScreenProps> = ({
  ...
  const [editingItem, setEditingItem] = useState<WishItemWithMetadata | undefined>();
  ...
  const convertLegacyWishes = (legacyText: string): WishItemWithMetadata[] => {
    ...
    return wishLines.map((wishText, index) => {
      // Check if the wish contains a URL (look for 🔗 or http/https)
      ...
      return {
        ...
        createdAt: Date.now() - (wishLines.length - index) * 1000, // Stagger creation times
        updatedAt: Date.now(),
      };
    });
  };
  ...
  useEffect(() => {
    ...
    if (userWishList.length === 0 && data.gifts?.[currentUser]) {
      // Convert legacy wishes to new format
      const legacyWishes = data.gifts[currentUser];
      const convertedWishes = convertLegacyWishes(legacyWishes);
      setWishList(convertedWishes);
    } else {
    ...
    if (recipientWishes.length === 0 && data.gifts?.[selectedName]) {
      // Convert legacy wishes to new format for display
      const legacyWishes = data.gifts[selectedName];
      const convertedWishes = convertLegacyWishes(legacyWishes);
      setRecipientWishList(convertedWishes);
    } else {
      setRecipientWishList(recipientWishes);
    }
  }, [data.wishLists, data.gifts, currentUser, selectedName]);

  const handleSaveWish = async (wishData: Omit<WishItemWithMetadata, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }) => {
    const now = Date.now();
    ...
    if (wishData.id) {
      ...
      updatedWishList = wishList.map(item => 
        item.id === wishData.id 
          ? { ...wishData, id: wishData.id, createdAt: item.createdAt, updatedAt: now } as WishItemWithMetadata
          : item
      );
    } else {
      // Add new wish
                    ...
                    {item.purchasedBy ? (
                      <div className="flex items-center space-x-2">
                        ...
                        {item.purchasedBy === currentUser && (
                          <button
                            onClick={() => handlePurchaseToggle(selectedName, item.id)}

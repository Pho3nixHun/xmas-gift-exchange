...
const API_HEADERS = {
  'X-Master-Key': '$2b$10$KjA6bzM7uur9YFhXy99TUO9CTPHcHTGmhWx8h89B1OHt0uavJT2iW',
  'X-BIN-META': 'false',
...
export const setNamePickerData = (data: NamePickerData) => ApiService.updateData(data);
...
export class ApiService {
  ...
  static async pickName(user: string, selectedName: string): Promise<NamePickerData> {
  ...
  static async setWishList(user: string, wishList: WishList): Promise<NamePickerData> {
    const currentData = await this.fetchData();
  ...
  static async setPassword(user: string, password: string): Promise<NamePickerData> {
    const currentData = await this.fetchData();
    
    const updatedData = {
      ...currentData,
      passwords: { ...currentData.passwords, [user]: password }
    };
